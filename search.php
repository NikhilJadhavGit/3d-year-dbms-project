	...
	<article>
		...
		<?php
			if(isset($_GET['search']))
			{
				$count=0;
			$spaces=0;
			$result=mysqli_query($conn,"SELECT images.path,products.name,products.price,products.model, products.link  FROM images,products,product_images WHERE products.p_id=product_images.p_id AND images.i_id=product_images.i_id AND product_images.p_id=product_images.i_id AND 
			 ( products.tag like '%".$_GET['search']."%' 
			 OR products.model like '%".$_GET['search']."%'
			  OR products.name like '%".$_GET['search']."%' 
			  OR products.price='".$_GET['search']."' )");
			...
			if($count==0)
			{
				echo "no results present for search keyword ".$_GET['search']." !!";
			}
